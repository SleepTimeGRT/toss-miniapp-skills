---
description: "토스페이(TossPay) 관련 질문이나 작업 시 이 스킬을 사용하세요. 다음 상황에서 트리거됩니다: 토스페이 소개/이해, 토스페이 콘솔 설정(가맹점 키 등록, 계약), 결제 연동 개발(make-payment, execute-payment, refund-payment, get-payment-status API), 결제 생성/인증/승인/환불/조회 구현, checkoutPayment SDK 연동, 결제상태 코드/은행코드/카드사코드 조회, QA 체크리스트 확인, 에러 코드 처리"
---
