...
- **앱마켓 수수료:** 공급가 기준 15% 또는 30% (현재 15% 적용, 총 수익 증가 시 30%로 변경 가능)
- **토스 수수료:** 결제 금액 기준 5%
...
- 토스가 애플 대신 세금계산서를 발행해요.
- 애플 수수료에 대한 부가세는 애플 정산금에서 먼저 공제돼요. 공제된 부가세는 추후 환급받을 수 있어요.
...
> 월 단위 정산 내역을 기준으로 금액을 보정한 뒤 발행하므로, 건별 합산과 차이가 날 수 있어요.
...
- 주문별로 기프트카드 결제 여부를 확인한 뒤, 결제 상태에 따라 현금 영수증을 발급하거나 취소해 주세요.
  ...
  - 앱마켓 직권 취소 등으로 정산금에 포함되지 않는 환불 건은 결제 완료 건으로 처리해야 해요. 이 경우 현금 영수증을 취소하면 안 돼요.
...
프로모션에서 토스 포인트를 지급하거나 광고 푸시를 발송할 때 사용해요. 현재는 신용카드(법인카드 포함)로만 충전할 수 있어요.

#### 사용 내역

나의 워크스페이스 > 좌측 비즈 월렛 메뉴에서 당월 충전/환불/사용/환급 내역과 현재 사용 가능 금액을 실시간으로 확인할 수 있어요.

- 사용 항목은 마케팅이 등록되는 시점에 업데이트돼요. 실제 사용자 포인트 수령 여부는 인보이스에서 확인해요.
- 환급 항목은 마케팅 등록 시 예상 예산과 실사용 예산의 차액이에요. 마케팅 종료 후 최대 1~2일 이내에 환급돼요.
- 사용 가능 금액 = `충전 금액 - (환불 금액 + 사용 금액) + 환급 금액` (인보이스 금액과 차이가 날 수 있어요)

#### 인보이스

나의 워크스페이스 > 좌측 비즈 월렛 메뉴 > 인보이스 보기에서 월 단위 정산 기준 정보를 확인할 수 있어요. 익월 1~2일 사이에 업데이트돼요.
...
- 프로모션이 끝나면 실사용 예산 기준으로 재계산 후 남은 금액은 자동 환급돼요.
- 토스 포인트는 무상 지급이므로 부가세와 세금계산서는 발생하지 않아요.
...
- 등록 시: `예상 발송 건수 x 건당 수수료` 기준으로 비즈 월렛에서 선차감
- 발송 완료 후: `성공 건수 x 건당 수수료` 기준으로 재계산, 차액 자동 환급

| 발송 건수 | 건당 수수료 | 건당 부가세 | 건당 수수료(부가세 포함) | 발송 비용 |
| --------- | ----------- | ----------- | ------------------------ | --------- |
| 100       | ₩9          | ₩0.9        | ₩9.9                     | ₩990      |

세금계산서는 실제 발송 성공 건수 기준으로 매월 3영업일까지 발행해요.
